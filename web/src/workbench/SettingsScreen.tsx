...
import { BORDER2, PageHeader, SUB } from './Shell';
...
function HeaderButton({ children, onClick, kind = 'plain', disabled, title }: {
...
function Fade({ on, children }: { on: boolean; children: ReactNode }) {
  return <div style={{ display: 'flex', flexDirection: 'column', gap: 24, opacity: on ? 1 : 0.45, transition: 'opacity .2s ease' }}>{children}</div>;
}

function Row({ label, help, stacked, aside, children }: { label: string; help?: string; stacked?: boolean; aside?: string; children: ReactNode }) {
  const text = (
...
function Master({ label, help, on, onChange }: { label: string; help: string; on: boolean; onChange(): void }) {
  return (
...
function Stepper({ value, max, onChange, label }: { value: number; max: number; onChange(v: number): void; label: string }) {
  ...
  return (
    <span role="group" aria-label={label} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      {btn(value > 0, 'Fewer', 'minus.circle.fill', value - 1)}
...
function TextInput({ value, onChange, placeholder, label }: { value: string; onChange(v: string): void; placeholder: string; label: string }) {
  return (
    <input className="field" aria-label={label} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder}
      style={{ width: 280, height: 34, borderRadius: 8, border: '1px solid rgba(60,60,67,0.14)', padding: '0 10px', fontSize: 14, outline: 'none', background: '#fff' }} />
  );
}

/** Digits only, up to 3. Blank is ''. */
function NumInput({ value, onChange, label, placeholder, width, block, maxWidth, fill = CANVAS, color = C.blue }: {
