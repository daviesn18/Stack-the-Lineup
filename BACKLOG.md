...
> **Account-level tour — fixed 21 Aug 2026 (from a coach report, not a numbered item).** The contextual tour was replaying for every new team: its TipKit gating `@Parameter`s (`TourState`) mirror the **active** team, so a fresh team reset `hasArchivedGame` to false and re-armed all of arc 1, walking an experienced coach through setup again. The tour is a first-team experience. `TipsConfigurator.suppressTourForMultiTeamCoach(teamCount:)` (called from `syncTourState`) now retires the whole tour — `suppressArcOne()` + a new `suppressArcTwo()`, the same account-level `invalidate(reason:)` datastore mechanism that already grandfathers experienced coaches — the first time it sees `teamCount > 1`. It runs on launch, so an existing multi-team account is fixed immediately, not just future teams. The `didAutoSuppressTourForMultiTeam` latch is deliberately **not** cleared by `restartTour()`, so "Take the Tour" still wins; single-team first-timers are untouched. Verified on the simulator (a 4-team install sets both flags on launch).

State of the repo: `main` is at `643dc24` and **pushed**. Today's landings (21 Aug), newest first: `643dc24` the account-level tour fix, `0ddd47c` 3.4 (TipKit live advance) + tour-copy edits, `d053821` 3.2 (CloudKit push debounce), `153ab3d` 4.6 (App Intents SDK check), `b283804`/`6baa3da` 4.3 (paywall Dynamic Type) + docs.

**`feature/app-intents-phase-0` is gone**, deleted 20 Aug 2026 — `git branch --merged` showed it fully contained in `main` with a zero-line diff, so the decision it was waiting on had already made itself. `fix/pitching-window-sunday` and `claude/little-league-pitching-rules-hn7237` went the same way for the same reason. Suite green — **346 of 346**, measured 21 Aug (322 when 3.7 restored it on 9 Aug; +18 since, the latest six being 3.2's `CloudPushDebouncerTests`). Static analyzer clean. A Release build emits 14 warnings, all pre-existing and none blocking — see the correction under 1.1 and item 4.5. The Worker lives in its own repo now — [`daviesn18/stl-worker`](https://github.com/daviesn18/stl-worker), private — with its own README covering the push architecture.
